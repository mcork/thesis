...
\newline
Most networking is based on two main types of connection; server-client connections and node based connections. The server client method is one source
\newpage
...
There is currently no benchmark for the size of data which corruption is reliable. The first experiment was to test structures of different sizes with a circular buffer of size one. The purpose of this experiment was to show at what size corruption first occurs and to develop a benchmark to determine the size that corruption reliably occurs. This benchmark allows comparison to be made.\newline
The experiment is consist of two programs accessing the shared memory. The first program will be changing the array and writing it to shared memory. The other program will be reading from shared memory and checking that all the contents are the same integer.\newline
\underline {\bf Method}
\begin{enumerate}
\item Change Array Size
\item Run Both Programs
...
The experiment will start of using an array the size determined in experiment 1.1 as the benchmarking value. The testing procedure will be similar to the method used in experiment 1.1. The circular buffer size will be increased and the test re-run. The experiment will be repeated until the test shows that the corruption has been minimised of eliminated completely.\newline
\underline {\bf Method}
\begin{enumerate}
\item Change Array Size to benchmark size
